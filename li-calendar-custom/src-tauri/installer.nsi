...
VIAddVersionKey "ProductName" "${SHORTCUTNAME}"
VIAddVersionKey "FileDescription" "${SHORTCUTNAME}"
...
Section Install
  ...
  !insertmacro CheckIfAppIsRunning "${MAINBINARYNAME}.exe" "${SHORTCUTNAME}"
  ...
  {{#each file_associations as |association| ~}}
    {{#each association.ext as |ext| ~}}
       !insertmacro APP_ASSOCIATE "{{ext}}" "{{or association.name ext}}" "{{association-description association.description ext}}" "$INSTDIR\${MAINBINARYNAME}.exe,0" "Open with ${SHORTCUTNAME}" "$INSTDIR\${MAINBINARYNAME}.exe $\"%1$\""
    {{/each}}
  {{/each}}
